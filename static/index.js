...
function createChartEight() {
    ...
    var options = {
        ...
        title: {
            text: 'Rest, Active, and Awake Averages per Region'
        },
        subtitle: {
            text: document.ontouchstart === undefined ?
                'Click and drag in the plot area to zoom in' : 'Pinch the chart to zoom in'
    ...
    for (i = 0; i < regions.length; i++) {
        ...
        processed_json_rest.push([regions[i], rest_average]);
        processed_json_active.push([regions[i], active_average]);
        processed_json_awake.push([regions[i], awake_average]);
    }
    // create the chart
    options.series[0].data = processed_json_rest;
    options.series[1].data = processed_json_active;
